...
const handleIssue = async (github, context) => {
    ...
    const tgTokenApi = process.env.TG_TOKEN_API;
    if (/^Order ORDER-\d{8}-[A-Z0-9]{6}$/.test(title)) {
        await new order_1.default(github, context, tgTokenApi, process.env.PRIVATE_KEY, process.env.WALLET_MNEMONIC).start();
        return { order: true };
    }
    else if (title.toLowerCase() === 'product' || title.toLowerCase() === 'category') {
        return await new product_1.default(github, context, tgTokenApi, title.toLowerCase()).start();
    }
